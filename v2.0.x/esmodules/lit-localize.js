...
const defaultMsg = (template)=>isStrTagged1(template) ? joinStringsAndValues1(template.strings, template.values) : template
;
const hl = [];
for(let i = 0; i < 256; i++){
...
class LocalizeController {
    ...
    hostConnected() {
        window.addEventListener(LOCALE_STATUS_EVENT1, this.__litLocalizeEventHandler);
    }
    hostDisconnected() {
        window.removeEventListener(LOCALE_STATUS_EVENT1, this.__litLocalizeEventHandler);
    }
}
const _updateWhenLocaleChanges = (host1)=>host1.addController(new LocalizeController(host1))
;
...
function dispatchStatusEvent(detail) {
    window.dispatchEvent(new CustomEvent(LOCALE_STATUS_EVENT1, {
        detail
    }));
}
let msg1 = defaultMsg;
...
const setLocale = (newLocale)=>{
    ...
    const localePromise = newLocale === sourceLocale ? Promise.resolve({
        templates: undefined
    ...
    if (templates) {
        ...
        if (localized2) {
            if (typeof localized2 === "string") {
                ...
                return joinStringsAndValues1(localized2.strings, template.values, localized2.values);
            } else {
