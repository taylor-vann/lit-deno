...
const supportsAdoptingStyleSheets1 = window.ShadowRoot && (extraGlobals.ShadyCSS === undefined || extraGlobals.ShadyCSS.nativeShadow) && "adoptedStyleSheets" in Document.prototype && "replace" in CSSStyleSheet.prototype;
...
const unsafeCSS1 = (value)=>new CSSResult1(typeof value === "string" ? value : String(value), constructionToken)
;
const css1 = (strings, ...values)=>{
    const cssText1 = strings.length === 1 ? strings[0] : values.reduce((acc, v, idx)=>acc + textFromCSSResult(v) + strings[idx + 1]
    , strings[0]);
...
const getCompatibleStyle1 = supportsAdoptingStyleSheets1 ? (s)=>s
...
if (true) {
    ...
    if (window.ShadyDOM?.inUse && globalThis["reactiveElementPlatformSupport"] === undefined) {
...
class ReactiveElement1 extends HTMLElement {
    ...
    __propertyToAttribute(name, value, options = defaultPropertyDeclaration) {
        ...
        if (attr !== undefined && options.reflect === true) {
            const toAttribute = options.converter?.toAttribute ?? defaultConverter1.toAttribute;
            const attrValue = toAttribute(value, options.type);
            if (true && this.constructor.enabledWarnings.indexOf("migration") >= 0 && attrValue === undefined) {
    ...
    _$attributeToProperty(name, value) {
        ...
        if (propName !== undefined && this.__reflectingProperty !== propName) {
            ...
            const fromAttribute = (converter?.fromAttribute ?? (typeof converter === "function" ? converter : null)) ?? defaultConverter1.fromAttribute;
    ...
    _$didUpdate(changedProperties) {
        ...
        if (true && this.isUpdatePending && this.constructor.enabledWarnings.indexOf("change-in-update") >= 0) {
...
const wrap = true && extraGlobals1.ShadyDOM?.inUse && extraGlobals1.ShadyDOM?.noPatch === true ? extraGlobals1.ShadyDOM.wrap : (node)=>node
;
...
const isIterable = (value)=>isArray(value) || typeof value?.[Symbol.iterator] === "function"
;
const SPACE_CHAR = `[ \t\n\\r]`;
...
if (true) {
    ...
    if (true) {
        render1._testOnlyClearSanitizerFactoryDoNotCallOrElse = _testOnlyClearSanitizerFactoryDoNotCallOrElse;
    }
}
...
const getTemplateHtml = (strings, type)=>{
    ...
    for(let i = 0; i < l; i++){
        ...
        html1 += regex === textEndRegex ? s + nodeMarker : attrNameEndIndex >= 0 ? (attrNames.push(attrName), s.slice(0, attrNameEndIndex) + boundAttributeSuffix + s.slice(attrNameEndIndex)) + marker + end : s + marker + (attrNameEndIndex === -2 ? (attrNames.push(undefined), i) : end);
    }
    const htmlResult = html1 + (strings[l] || "<?>") + (type === 2 ? "</svg>" : "");
    return [
                    ...
                    for (const name of node1.getAttributeNames()){
                        if (name.endsWith(boundAttributeSuffix) || name.startsWith(marker)) {
                            ...
                            if (realName !== undefined) {
                                const value = node1.getAttribute(realName.toLowerCase() + boundAttributeSuffix);
        ...
        while(templatePart !== undefined){
            if (nodeIndex1 === templatePart.index) {
                ...
                if (templatePart.type === 2) {
                    ...
                    part = new templatePart.ctor(node1, templatePart.name, templatePart.strings, this, options);
...
class ChildPart {
    ...
    _commitText(value) {
        ...
        if (node2 !== null && node2.nodeType === 3 && (this._$endNode === null ? wrap(node2).nextSibling === null : node2 === wrap(this._$endNode).previousSibling)) {
            if (true) {
    ...
    _commitTemplateResult(result) {
        ...
        const template1 = typeof _$litType$ === "number" ? this._$getTemplate(result) : (_$litType$.el === undefined && (_$litType$.el = Template.createElement(_$litType$.h, this.options)), _$litType$);
...
class AttributePart {
    ...
    _$setValue(value, directiveParent = this, valueIndex, noCommit) {
        ...
        if (strings3 === undefined) {
            ...
            change = !isPrimitive(value) || value !== this._$committedValue && value !== noChange1;
            if (change) {
                this._$committedValue = value;
            }
        } else {
...
class EventPart extends AttributePart {
    ...
    _$setValue(newListener, directiveParent = this) {
        newListener = resolveDirective(this, newListener, directiveParent, 0) ?? nothing1;
        if (newListener === noChange1) {
            return;
        }
        const oldListener = this._$committedValue;
        const shouldRemoveListener = newListener === nothing1 && oldListener !== nothing1 || newListener.capture !== oldListener.capture || newListener.once !== oldListener.once || newListener.passive !== oldListener.passive;
        const shouldAddListener = newListener !== nothing1 && (oldListener === nothing1 || shouldRemoveListener);
    ...
    LitElement1["finalize"] = function() {
        ...
        const warnRemoved = (obj, name1)=>{
            if (obj[name1] !== undefined) {
                console.warn(`\`${name1}\` is implemented. It ` + `has been removed from this version of LitElement. `);
            }
        };
        [
            `render`,
...
;
const decorateProperty1 = ({ finisher , descriptor  })=>(protoOrDescriptor, name1)=>{
        if (name1 !== undefined) {
...
const customElement1 = (tagName)=>(classOrDescriptor)=>typeof classOrDescriptor === "function" ? legacyCustomElement(tagName, classOrDescriptor) : standardCustomElement(tagName, classOrDescriptor)
;
export { customElement1 as customElement };
const standardProperty = (options5, element3)=>{
    if (element3.kind === "method" && element3.descriptor && !("value" in element3.descriptor)) {
        return {
            ...element3,
            finisher (clazz) {
...
function property1(options5) {
    return (protoOrDescriptor, name1)=>name1 !== undefined ? legacyProperty(options5, protoOrDescriptor, name1) : standardProperty(options5, protoOrDescriptor)
    ;
}
    ...
    return decorateProperty1({
        descriptor: (_name)=>({
                get () {
                    ...
                    if (nodes && selector) {
                        nodes = nodes.filter((node2)=>node2.nodeType === Node.ELEMENT_NODE && (node2.matches ? node2.matches(selector) : legacyMatches.call(node2, selector))
                        );
                    }
                    return nodes;
                },
