...
const supportsAdoptingStyleSheets = window.ShadowRoot && (extraGlobals.ShadyCSS === undefined || extraGlobals.ShadyCSS.nativeShadow) && "adoptedStyleSheets" in Document.prototype && "replace" in CSSStyleSheet.prototype;
...
const getCompatibleStyle = supportsAdoptingStyleSheets ? (s)=>s
...
class ReactiveElement extends HTMLElement {
    ...
    _$attributeToProperty(name, value) {
        ...
        if (propName !== undefined && this.__reflectingProperty !== propName) {
            ...
            const fromAttribute = (converter?.fromAttribute ?? (typeof converter === "function" ? converter : null)) ?? defaultConverter.fromAttribute;
...
const getTemplateHtml = (strings, type)=>{
    ...
    const htmlResult = html + (strings[l] || "<?>") + (type === 2 ? "</svg>" : "");
    return [
...
const decorateProperty = ({ finisher , descriptor  })=>(protoOrDescriptor, name1)=>{
        if (name1 !== undefined) {
...
export { css as css, html as html, LitElement1 as LitElement };
